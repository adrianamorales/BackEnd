...
<?php
  ...
  if( isset($_POST['name']) ){
    $stmt = $conn->prepare("insert into form (name, form, excel) values (:name, :form, :excel)");
    $stmt->bindParam(':name', $name);
    $stmt->bindParam(':form', $form);
    $stmt->bindParam(':excel', $excel);
    $form = $_POST['form'];
    $name = $_POST['name'];
    $excel = $_POST['excel'];
    $stmt->execute();
  ...
  if( isset($_POST['fid']) ){
    $stmt = $conn->prepare("DELETE FROM form where fid= :fid");
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon edit"></i><span class="break"></span>Forms</h2>
						<div class="box-icon">
...
<?php
  ...
  {
    echo "<p><a href=\"" . $results['form'] . "\">" . $results['name'];
    echo "</a></p>";
    if ($admin == true) {
      echo "<ul><li>";
      echo "<p><a href=\"";
      echo $results['excel'];
      echo "\">click here for the results</a></p>";
      echo "</li>";
...
<?php
if ($admin == true) {
    echo '<h2>Add a Form</h2>';
    echo '<p>';
    echo '<form action="form.php" method="post">';
    echo '<input class="input-large" name="name" id="name" type="text" placeholder="name of form" required/><br />';
    echo '<input class="input-large" name="form" id="form" type="text" placeholder="url to form" required/><br />';
    echo '<input class="input-large" name="excel" id="excel" type="text" placeholder="url to excel results sheet" required/><br />';
    echo '<button type="submit" class="btn btn-primary">Add Form</button>';
    echo '</form>';
    echo '</p>';
}
?>
