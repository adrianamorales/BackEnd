...
<?php
  if( isset($_POST['email2']) ){
    ...
    if( strlen($_POST['first']) > 2){
      $sql = "UPDATE info set first = :first where email= :email";
    ...
    if( strlen($_POST['last']) > 2){
      $sql = "UPDATE info set last = :last where email= :email";
      $stmt = $conn->prepare($sql);
    ...
    if( strlen($_POST['phone']) > 2){
      $sql = "UPDATE info set phone = :phone where email= :email";
      $stmt = $conn->prepare($sql);
    ...
    if( isset($_POST['password'])){
      ...
      $stmt->execute();
      //update hash
      $sql = "UPDATE user set salt = :salt where email= :email";
      $stmt = $conn->prepare($sql);
    ...
    if( isset($_POST['newemail'])){
      $sql = "UPDATE info set email = :newemail where email= :email";
      $stmt = $conn->prepare($sql);
  ...
  if( isset($_POST) ){
    ...
    $stmt = $conn->prepare("select first, last, email, phone from info where email = :email");
    ...
    foreach ($row as $result){
      echo "<h2>Update Contact Info</h2>";
      echo "<p>";
      echo '<form action="admin.php" method="post">';
      echo 'Email: <input type="email" name="newemail" placeholder="';
      echo $result['email'];
      echo '"/><br />';
      echo 'First Name: <input type="text" name="first" placeholder="';
      echo $result['first'];
      echo '"/><br />';
      echo 'Last Name: <input type="text" name="last" placeholder="';
      echo $result['last'];
      echo '"/><br />';
      echo 'Phone Number: <input type="text" name="phone" maxlength="12" placeholder="';
      echo $result['phone'];
      echo '"/><br />';
    ...
    echo '<form action="admin.php" method="post">';
    echo 'Password: <input type="text" name="password" placeholder="Type New Password" required/>';
    echo '<input type="hidden" name="email2" value="';
    echo $email;
    echo '"/>';
    echo '<br /><button type="submit" class="btn btn-primary">Change Password</button>';
    echo "</form>";
  }
